...
function [TC_OUT,param] = RunTotalActivation(TCN,param)
    ...
    while (k <= param.Nit)
        ...
        if ~isfield(param, 'weight_x'), param.weight_x = ones(param.Dimension(1:3)); end
        if ~isfield(param, 'weight_y'), param.weight_y =  ones(param.Dimension(1:3)); end
        if ~isfield(param, 'weight_z'), param.weight_z =  ones(param.Dimension(1:3)); end
        ...
        Op  =  @(x)gradient3D_full(x,wx,wy,wz);          % Gradient operator
        Adj_Op = @(u,v,w)-div3D_full(u,v,w,wx,wy,wz);    % Its Adjoint : minus divergence <f,grad g> = -<div f,u>
        evaluate_norm = @(y)evaluate_3D_TV(y);   % compute sum TV{y}
       ...
       if(k<param.Nit)
                fprintf('Launching Spatial_TV_Map\n');
                tms = toc;
                % calculates for the whole volume
                temp2 = TA_Spatial(TC_OUT-xS+TCN,param,Op,Adj_Op,evaluate_norm); % calculates for the whole volume
                fprintf('Spatial_TV_Map completed in %.5f sec\n', toc-tms);
                xS = xS+(temp2-TC_OUT);
            end
