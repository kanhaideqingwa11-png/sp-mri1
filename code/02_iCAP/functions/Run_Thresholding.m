...
function [] = Run_Thresholding(param)
    ...
    for iS = 1:param.n_subjects
        ...
        if ~Thresholding_done
            ...
            WriteInformation(fid,['Removing NaNs from innovations (surrogate)...']);
            [newI_surrogate,param.mask_nonan] = RemoveNan(Innovation_surrogate,param,fid);
            
	    % Nawal - Added to also remove NaNs from real innovations
 	    WriteInformation(fid,['Removing NaNs from innovations (real)...']);
            [newI] = RemoveNan(Innovation,param,fid);

            WriteInformation(fid,['Computing percentiles...']);
            param.PC = ComputeSurrogatePercentiles(newI_surrogate,param,fid);

            WriteInformation(fid,['Selecting significant innovation frames...']);
            [SignInnov,param] = SelectSignificantFrames(newI,param,fid);
	    ...
	    save4Dnii(fullfile(resultsPath,'Thresholding'),param.thresh_title,'SignInnov',SignInnov',param.fHeader.fname,param.mask_nonan,param.Dimension);
            save4Dnii(fullfile(resultsPath,'Thresholding'),param.thresh_title,'mask_nonan',param.mask_nonan,param.fHeader.fname,param.mask_nonan,param.Dimension);
